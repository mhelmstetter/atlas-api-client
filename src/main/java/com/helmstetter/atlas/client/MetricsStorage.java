...
public class MetricsStorage {
	...
	/**
	 * Build a cache of the last timestamp for each host+metric combination This
	 * helps with efficiently checking for duplicates
	 */
...
// Convert value to double
			...
			if (value == null) {
				logger.warn("Null or invalid value for {}:{} metric {} at {}", host, port, metric, timestampStr);
				continue;
			}
...
// First, check if this exact data point already exists in the database
			...
			if (existingCount > 0) {
			    ...
			    logger.warn("Duplicate document found: {}", existingDoc);
			    skippedPoints++;
			    continue;
			}
...
// Update the cache with the latest timestamp
				...
				logger.info("Stored {} new data points for {}:{} metric {} (skipped {} duplicates)", newPoints, host,
						port, metric, skippedPoints);
			} catch (Exception e) {
				logger.error("Failed to store metrics for {}:{} metric {}: {}", host, port, metric, e.getMessage());
			}
		} else {
			logger.info("No new data points to store for {}:{} metric {} (all {} points were duplicates)", host, port,
